...
// Same as Sub Admin's Add Student, plus a Campus selector up front since
// Super Admin isn't locked to one campus - picking a campus narrows the
// batch list down to slots that actually belong to it.
function StudentAdd() {
  ...
  const set = (field) => (e) => {
    ...
    setForm((prev) => {
      ...
      if (field === 'campus' || field === 'course') next.slot = ''
      return next
    })
  }

  const availableSlots = useMemo(
    () => slots.filter((s) => s.course?._id === form.course && s.campus?._id === form.campus && s.registrationOpen && s.seatsUsed < s.capacity),
    [slots, form.course, form.campus]
  )
  ...
  const requiredFields = ['campus', 'course', 'slot', 'name', 'fatherName', 'email', 'phone', 'cnic', 'dob', 'gender']
        ...
        <div className="student-form-grid">
          ...
          <div className="auth-input-group">
            ...
            <div className={`auth-input-wrap ${errors.slot ? 'auth-input-wrap-error' : ''}`}>
              <select className="auth-input" value={form.slot} onChange={set('slot')} disabled={!form.course || !form.campus}>
                <option value="">Select</option>
            ...
            {form.course && form.campus && availableSlots.length === 0 && (
              <span className="subadmin-role-hint" style={{ color: '#C53030' }}>
                No open batches for this course at this campus yet — create one from Administration first.
              </span>
            )}
          </div>
          {selectedSlot && (
