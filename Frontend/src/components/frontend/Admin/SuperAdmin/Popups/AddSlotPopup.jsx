...
const emptyForm = {
  campus: '', course: '', trainer: '', scheduleDays: [], startTime: '', endTime: '', classType: 'Regular',
  gender: '', startDate: '', endDate: '', capacity: '', whatsappLink: '',
}

// Same as Sub Admin's Add Slot, plus a Campus selector since Super Admin can
// create a slot at any campus - picking one narrows the Trainer list down
// to trainers who actually belong to it.
function AddSlotPopup({ show, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    api.get('/teachers').then(setAllTrainers).catch(() => {})
  }, [show])
  ...
  const set = (field) => (e) => {
    ...
    setForm((f) => ({ ...f, [field]: value, ...(field === 'campus' ? { trainer: '' } : {}) }))
  }

  const trainersAtCampus = allTrainers.filter((t) => t.campus?._id === form.campus)
  ...
  const handleSubmit = async () => {
    setError('')
    const missing = []
    if (!form.campus) missing.push('Campus')
        ...
        <div className="edit-profile-grid">
          <div className="auth-input-group edit-profile-grid-full">
            ...
            <div className="auth-input-wrap">
              <select className="auth-input" value={form.campus} onChange={set('campus')}>
          ...
          <div className="auth-input-group">
            ...
            <div className="auth-input-wrap">
              <select className="auth-input" value={form.trainer} onChange={set('trainer')} disabled={!form.campus}>
                <option value="">Unassigned — assign later</option>
                {trainersAtCampus.map((t) => <option key={t._id} value={t._id}>{t.name}</option>)}
              </select>
            </div>
            {form.campus && trainersAtCampus.length === 0 && (
              <span className="subadmin-role-hint">No trainers at this campus yet — you can create this slot now and add a trainer to it afterwards.</span>
            )}
          </div>
